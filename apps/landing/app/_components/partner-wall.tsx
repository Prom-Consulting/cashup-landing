...
/** Пока витрина пуста, показываем, где бонусы принимают по смыслу — а не пустое место. */
const categories = [
...
const tile = "grid h-[76px] w-[76px] place-items-center overflow-hidden rounded-full bg-cream sm:h-[92px] sm:w-[92px]";

/**
 * Стена партнёров — первое, что видит человек: не обещание, а места, где бонусы
 * уже принимают. Данные живые, из публичной витрины бэкенда.
 *
 * Витрина отдаёт только заведения с действующей подпиской, и в начале жизни продукта
 * она пуста. Тогда показываем категории и зовём подключиться: первый экран не должен
 * зависеть от того, успел ли кто-то заполнить профиль.
 */
        ...
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            ...
            <p className="mt-4 max-w-[56ch] text-lg leading-snug text-slate-soft sm:text-xl">
              {empty
                ? "Каждый оплаченный месяц на карте 100 000 сом бонусами. Заведения подключаются прямо сейчас — первые появятся здесь."
                : "100 000 сом бонусами каждый оплаченный месяц — тратьте их здесь. Список пополняется, и каждое заведение само задаёт, какую часть чека закроют бонусы."}
            </p>
          </div>

          <Link
            href={empty ? "/become-partner" : "/partners"}
          ...
          >
            {empty ? "Подключить своё заведение" : "Все партнёры на карте"}
          </Link>
        </div>

        {empty ? (
          <ul className="mt-12 grid grid-cols-2 gap-x-4 gap-y-9 sm:grid-cols-3 lg:grid-cols-6">
            {categories.map((category) => (
              <li key={category.icon} className="flex flex-col items-center text-center">
                <span className={tile}>
          ...
          <ul className="mt-12 grid grid-cols-3 gap-x-4 gap-y-9 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
            {shown.map((partner) => (
              <li key={partner.id}>
                ...
                >
                  <span
                    className={`${tile} ring-0 ring-flame transition-[box-shadow] group-hover:ring-4 group-focus-visible:ring-4`}
                  >
                    {partner.logoUrl ? (
