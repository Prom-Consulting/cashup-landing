        ...
        {active && (
          <div className="pointer-events-none absolute top-4 right-4 left-4 rounded-2xl bg-forest p-4 text-chalk sm:top-auto sm:bottom-4 sm:left-auto sm:w-72">
            <p className="flex items-center gap-2 text-lg font-bold">
              {active.name}
              {active.octopay && (
                <span className="rounded-full bg-chalk px-2 py-0.5 text-xs text-magenta-ink">OctōPAY</span>
              )}
            </p>
            <p className="mt-1 text-sm opacity-85">
              {active.address} · {active.hours}
            </p>
            <p className="display mt-2 text-3xl text-bubblegum">до {active.percent}% бонусами</p>
          </div>
        )}
      </div>

      <div className="flex flex-wrap items-center gap-x-6 gap-y-2 border-t-2 border-blush px-5 py-4 text-sm">
        <span className="flex items-center gap-2">
          <span className="h-4 w-4 rounded-full rounded-bl-sm border-2 border-magenta bg-magenta-ink" />
          Списывает бонусы сам через OctōPAY
        </span>
        <span className="flex items-center gap-2">
          <span className="h-4 w-4 rounded-full rounded-bl-sm border-2 border-magenta-ink bg-chalk" />
