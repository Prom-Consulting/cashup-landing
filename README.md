...
В код зашит демо-ключ MapGL. Если 2GIS отвечает, что ключ неактивен, страница сама показывает
стилизованную схему города в фирменных цветах вместо карты.

Переопределить ключ и стиль можно через `.env.local` (шаблон — `.env.example`):

- `NEXT_PUBLIC_2GIS_KEY` — боевой ключ доступа из Platform Manager 2GIS;
