...
Подписка на бонусы: 17 $ в месяц — карта в Apple Wallet и 15 000 бонусов на каждый оплаченный месяц.
...
живут там же. Нужны пять записей типа `A` на адрес сервера — апекс, `www`, `admin`, `partner`,
`client`; шлюз бэкенда (`loal.promconsult.pro`) к этой зоне не относится.
